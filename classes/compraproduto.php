	...
	*/
	class CompraProduto
	{
		...
		public function insert($InputCompra, $InputFornecedor,$InputProduto, $InputQtd_prod, $InputNome_prod){
			...
			echo '<script>window.location="painel_list_compra.php?lista=5&pagina=1";</script>';
			$stmt->close();
			$db_conn->close();
        	...
        	if($stmt->execute()) {
        		...
        		while($stmt->fetch()) {
        			...
        			printf("<td>" .
        				...
        				$nm_prod
        				. "</td><td>" . 
        				"<a class='btn btn-success modalLink' href='#update_prod' data-id=".$cd_copr." data-toggle='modal' data-target='#update_prod'><em class='fa fa-pencil'></em></a>
